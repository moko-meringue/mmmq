...
public record TopicStorageProperties(
        String dataDir,       // 세그먼트 파일을 저장할 루트 디렉토리 경로
        long segmentMaxBytes  // 세그먼트 파일 최대 크기 (바이트). 초과 시 새 세그먼트로 회전
) {

    private static final String DEFAULT_DATA_DIR = "./data"; // dataDir 미설정 시 사용하는 기본 경로
    ...
    public TopicStorageProperties { // compact 생성자: 바인딩 직후 실행되어 누락된 값에 기본값을 채움
        if (dataDir == null || dataDir.isBlank()) { // dataDir가 null이거나 공백 문자열이면 기본값으로 대체
