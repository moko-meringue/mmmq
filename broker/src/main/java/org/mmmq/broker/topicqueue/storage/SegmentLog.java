...
final class SegmentLog implements Closeable { // .log 파일 한 개를 캡슐화. append-only 쓰기와 position 기반 읽기만 허용
    ...
    private SegmentLog(Path path, FileChannel channel) { // 외부에서 직접 생성하지 않도록 생성자를 private으로 제한
        this.path = path;
        this.channel = channel;
    }

    static SegmentLog openOrCreate(Path path) { // 파일이 없으면 생성, 있으면 기존 내용을 유지하며 열기
        try {
            ...
            return new SegmentLog(path, channel);
        } catch (IOException exception) {
            throw new StorageException("Failed to open segment log: " + path, exception);
        }
    }
    ...
    long appendAndForce(byte[] payload) { // payload를 파일 끝에 쓰고 fsync를 완료한 뒤 기록 시작 위치를 반환
        try {
            ...
            throw new StorageException("Failed to append to segment log: " + path, exception);
        }
    }
    ...
    byte[] readLineAt(long position) { // position부터 '\n'까지 bytes를 읽어 반환. '\n' 포함
        try {
            ...
            if (position < 0 || position >= fileSize) { // 유효 범위 밖 위치 접근은 즉시 실패
                throw new StorageException("Read position out of bounds: " + position + " for log: " + path);
            }
            ...
            throw new StorageException("Failed to read from segment log: " + path, exception);
        }
    }
    ...
    long size() { // 현재 파일 크기를 반환. shouldRotate 판단과 recoverActiveSegment에서 사용
        try {
            ...
            throw new StorageException("Failed to read size of segment log: " + path, exception);
        }
    }
    ...
    void truncate(long newSize) { // 파일을 newSize bytes로 잘라내고 fsync. 부팅 복구 시 미커밋 trailing bytes 제거에 사용
        try {
            ...
            throw new StorageException("Failed to truncate segment log: " + path, exception);
        }
    }
    ...
    public void close() { // 사용 완료 후 FileChannel을 닫아 fd 누수 방지
        try {
            ...
            throw new StorageException("Failed to close segment log: " + path, exception);
        }
    }
}
