...
final class MessageCodec { // 외부에서 직접 인스턴스화할 필요가 없는 정적 유틸 클래스
    ...
    private static final byte LINE_TERMINATOR = (byte) '\n'; // 각 엔트리의 끝을 나타내는 구분자. readLineAt이 이 바이트를 만나면 읽기를 멈춤
    ...
    static byte[] encode(Message message)
        ...
        try {
            ...
            line[body.length] = LINE_TERMINATOR; // 배열 마지막 바이트에 줄 구분자 추가
            return line;
    ...
    static Message decode(byte[] line) throws MessageCodecException { // '\n'이 포함된 bytes를 읽어 Message 객체로 역직렬화
        try {
