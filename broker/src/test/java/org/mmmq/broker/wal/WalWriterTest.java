...
class WalWriterTest {
    ...
    void appendsMultipleEntriesToSameSegment(@TempDir Path tempDir) throws Exception {
        final WalWriter writer = new WalWriter(tempDir, "order", new PageCacheFlushPolicy());

        writer.write(new WalEntry(new Message(new Topic("order"), Map.of("id", 1))), 0);
        writer.write(new WalEntry(new Message(new Topic("order"), Map.of("id", 2))), 0);
    ...
    void rotatesToNewSegmentFile(@TempDir Path tempDir) throws Exception {
        ...
        writer.write(new WalEntry(new Message(new Topic("order"), Map.of("id", 1))), 0);
        writer.write(new WalEntry(new Message(new Topic("order"), Map.of("id", 2))), 1);
    ...
    void writeAfterDeleteRecreatesFile(@TempDir Path tempDir) throws Exception {
        ...
        writer.write(new WalEntry(new Message(new Topic("order"), Map.of("id", 3))), 0);
