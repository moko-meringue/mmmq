...
class WalRecoveryTest {
    ...
    void recoversTopicsInSegmentOrder(@TempDir Path walDir) throws Exception {
        ...
        final TopicQueue topicQueue = new TopicQueue(new Topic("order"), new NoOpWalWriter());
        ...
        assertThat(((java.util.Map<?, ?>) topicQueue.poll(offset).content()).get("id")).isEqualTo(2);
        assertThat(((java.util.Map<?, ?>) topicQueue.poll(offset).content()).get("id")).isEqualTo(3);
