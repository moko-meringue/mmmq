...
class OffsetStoreTest {
    ...
    void newStoreReturnsZero(@TempDir Path tempDir) {
        try (OffsetStore store = OffsetStore.openOrCreate(tempDir.resolve("a.offset"))) { // 파일 없음 → 0으로 초기화
    ...
    void writeAndRead(@TempDir Path tempDir) {
        try (OffsetStore store = OffsetStore.openOrCreate(tempDir.resolve("a.offset"))) {
            store.writeAndForce(123L); // 123을 디스크에 fsync
    ...
    void persistsAcrossInstances(@TempDir Path tempDir) {
        final Path file = tempDir.resolve("a.offset");
        try (OffsetStore store = OffsetStore.openOrCreate(file)) {
            store.writeAndForce(42L); // 42를 디스크에 fsync 완료
        } // 채널 닫기: 브로커 종료 시뮬레이션

        try (OffsetStore reopened = OffsetStore.openOrCreate(file)) { // 새 인스턴스로 같은 파일 열기: 재시작 시뮬레이션
