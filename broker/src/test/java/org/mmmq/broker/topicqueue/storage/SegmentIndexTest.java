...
class SegmentIndexTest {
    ...
    void countMatchesByteSize(@TempDir Path tempDir) {
        try (SegmentIndex index = SegmentIndex.open(tempDir, "test")) {
    ...
    void readAddressRoundTrip(@TempDir Path tempDir) {
        try (SegmentIndex index = SegmentIndex.open(tempDir, "test")) {
            ...
            assertThat(index.readAddressAt(0)).isEqualTo(0L);   // 첫 엔트리의 .mmm 주소
            assertThat(index.readAddressAt(1)).isEqualTo(100L); // 두 번째 엔트리의 .mmm 주소
            assertThat(index.readAddressAt(2)).isEqualTo(200L); // 세 번째 엔트리의 .mmm 주소
        }
    }
