...
class CheckpointTest {
    ...
    void newStoreReturnsZero(@TempDir Path tempDir) {
        try (Checkpoint store = Checkpoint.open(tempDir, "dispatcher-a")) { // 파일 없음 → 0으로 초기화
    ...
    void writeAndRead(@TempDir Path tempDir) {
        try (Checkpoint store = Checkpoint.open(tempDir, "dispatcher-a")) {
    ...
    void persistsAcrossInstances(@TempDir Path tempDir) {
        ...
        try (Checkpoint reopened = Checkpoint.open(tempDir, "dispatcher-a")) { // 새 인스턴스로 같은 파일 열기: 재시작 시뮬레이션
